...
import { FaSave, FaPhone, FaMapMarker, FaBuilding, FaEdit } from "react-icons/fa";

// API configuration
const API_BASE_URL = 'http://localhost:5000';

export default function BuyerProfile({ user, onLogout, notifications }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/api/buyer/${user.email}`)
      .then((res) => {
        const data = res.data;
        setProfile({
          name: data.name || "",
  ...
  const saveProfile = async () => {
    ...
    setIsSaving(true);
    try {
      await axios.post(`${API_BASE_URL}/api/buyer`, profile);

      const res = await axios.get(`${API_BASE_URL}/api/buyer/${user.email}`);
      ...
      <div className="profile-header">
        <div className="header-content">
          ...
          <div className="user-info">
            <h1 className="user-name">{profile.name || "Buyer"}</h1>
            <p className="user-email">{profile.email}</p>
            {/* Removed "Business Professional" tag */}
          </div>
          <button 
          ...
          <div className="info-section">
            ...
            <div className="info-cards">
              <div className="info-card">
                ...
                <div className="card-content">
                  <h3>Business Name</h3>
                  <p>{profile.business_name || "Not specified"}</p>
                </div>
              </div>
              <div className="info-card">
                <div className="card-icon">
                ...
                <div className="card-content">
                  <h3>Business Type</h3>
                  <p>{profile.business_type || "Not specified"}</p>
                </div>
              </div>
            </div>
          </div>
